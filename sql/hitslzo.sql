...
explain select day,ticker,max(lastPrice) as High ,min(lastPrice) as Low  from   hitslzo WHERE ticker="IBM" AND lastPrice IS NOT NULL AND YEAR=2014 AND MONTH=07  GROUP by day,ticker ORDER by day;
select day,ticker,max(lastPrice) as High ,min(lastPrice) as Low  from   hitslzo WHERE ticker="IBM" AND lastPrice IS NOT NULL AND YEAR=2014 AND MONTH=07  GROUP by day,ticker ORDER by day;

-- High/Low IBM 1 year
explain select day,ticker,max(lastPrice) as High ,min(lastPrice) as Low  from   hitslzo WHERE ticker="IBM" AND lastPrice IS NOT NULL AND YEAR=2014  GROUP by day,ticker ORDER by day;
select day,ticker,max(lastPrice) as High ,min(lastPrice) as Low  from   hitslzo WHERE ticker="IBM" AND lastPrice IS NOT NULL AND YEAR=2014  GROUP by day,ticker ORDER by day;
